...
```
## How to Run Test
To run jasmine test, simply clone or download this repository to your local machine, and open the ```SpecRunner.html``` in your browser (Chrome or Firefox), then you should be able to see the passed test cases.

## How to Run Game
To run the run this game, simply clone or download this repository to your local machine. Please find out the ```index.html``` in ```app``` folder.
Open ```index.html``` in an latest version browser, such as Firefox, Chrome. You should be able to see the gameboard to be drawn. More specific instruction on How to play and Game rules can be find in [README.md](./app/README.MD) in ```app``` folder.
